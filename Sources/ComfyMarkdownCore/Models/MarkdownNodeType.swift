...
extension MarkdownNodeType: CustomStringConvertible {
    var description: String {
        ...
        case .list(let ordered, let start, _, let delimiter):
            return "(list ordered=\(ordered) start=\(start) delimiter=\(delimiter ?? "nil"))"
        case .listItem: return "(list_item)"
        case .codeBlock(let info, let literal):
            return "(code_block info=\(info ?? "nil") literal=\"\(literal)\")"
        ...
        case .customBlock(let onEnter, let onExit):
            return "(custom_block onEnter=\(onEnter ?? "nil") onExit=\(onExit ?? "nil"))"
        case .customInline(let onEnter, let onExit):
            return "(custom_inline onEnter=\(onEnter ?? "nil") onExit=\(onExit ?? "nil"))"
